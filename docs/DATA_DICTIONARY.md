...
## 出典の到達状態（2026-08-20 追加）

`npm run audit-urls` が書き出す `data/url-audit.csv` の `check_status` を
`generate-data.mjs` が候補へ結び、検索画面のカードに反映する。

- `client_error` / `server_error` / `network_error` / `timeout` のいずれか
  （2026-08-20時点で77件）→ カードに **「更新情報がありません」** と表示し、
  「一次情報あり」は出さない。掲載値と観測日はそのまま残す
- `access_limited`（bot遮断など）はリンク切れと区別し、表示を変えない
- 監査に無いURLは判定しない

`verification_status = needs_check` は別軸で、休館・改修・掲載値の再取得など
確認事項が残る会場に付く。カードには「要確認」と表示する。両方が同時に出ることもある。

**掲載値を消さない。** 出典が開けないことと、施設が無くなったことは別である。
