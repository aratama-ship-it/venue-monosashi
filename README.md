...
- `data/historical-events.csv`: 開催年ごとの過去会場台帳（199行）
- `data/candidate-venues.csv`: 全国の類似会場候補（58施設・47都道府県）
...
この版は公開前の調査版です。公式サイトで確認できた事項と、比較のための評価・推論を分離しています。
...
2026-07-30時点の監査値は、過去大会199行、JYYF全国大会・前身大会28件（1999〜2026）、JYYF地区大会106件（連盟発足後の2007〜2026）、ジュニア大会9件（第1回の2018〜2026）、WYYC 28年、候補58施設、47都道府県、施設区画100件、料金観測277件、時間区分の参考合計11件、運用観測50件、エラー0です。未確認警告1件はWYYC 1999の正確なホノルル会場名で、推測値を入れず空欄にしています。

現在はGitHub上の`TEAM_REVIEW`段階で、ChatGPT Sitesの所有者限定URLがあります。`art-monosashi.com`配下での一般公開はまだ行っていません。

## ローカルWeb検索

`web/`で`npm run dev`を実行すると、原本CSVを再生成して検索画面を起動します。地域・都道府県・面積・収容・天井高・駐車・固定舞台・練習利用・料金用途・運用情報の有無で候補を絞り込み、証拠量・基本料・規模・予約開始時期で並べ替えられます。「同じ貸出区画で満たす」を選ぶと、別区画の最大値と安い料金を誤って組み合わせません。検索画面の予算上限は、選択した用途に一致する「確認済みの1日施設基本料」を対象とします。時間区分から組み立てた参考合計は、明示的に選んだ場合だけ検索へ加え、冷暖房・警備・清掃等の未確認費を0円として扱いません。画面下部では、過去大会199記録を系列・年・会場名・都市から検索し、確認状態と出典へ戻れます。
...
GitHubへのpush、チーム確認、一般公開は別の操作です。日常変更はDraft PRで確認し、データ監査・lint・テスト・本番ビルドが通った後に`READY`へ進めます。`PUBLIC`への変更と`art-monosashi.com`配下へのデプロイは、開発者の明示確認後に行います。
