...
- Recorded only explicit capacity and area values. No ceiling or price value was inferred and all unreported conditions remain `要確認`.
- Wave 49 local verification passed: app-data generation, `npm run audit`, `npm run depth-report:write`, and `git diff --check` passed with only the pre-existing historical-record warning. Miyagi is now 16 candidates, 12 municipalities, and 79 spaces; national remaining deficits are 499 candidates and 180 spaces. Public deployment remains to be completed.
